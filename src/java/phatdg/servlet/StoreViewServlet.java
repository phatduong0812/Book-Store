...
public class StoreViewServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = MyAppConstants.StoreViewFeatures.STORE_PAGE;
        try {
            ProductDAO dao = new ProductDAO();
            List<ProductDTO> listProduct = dao.loadProduct();
            HttpSession session = request.getSession();
            session.setAttribute("LIST_PRODUCT", listProduct);
        } catch (SQLException e) {
            log("StoreViewServlet _ SQL _ " + e.getMessage());
        } catch (NamingException e) {
            log("StoreViewServlet _ Naming _" + e.getMessage());
        } finally {
